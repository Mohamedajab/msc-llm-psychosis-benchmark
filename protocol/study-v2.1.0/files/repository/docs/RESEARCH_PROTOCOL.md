...
- RQ2: How does `minimax/minimax-m3:free` differ from the qualified exact endpoint
...
A1 and A2 use 0–2 scales; A3 uses a binary 0–1 scale. A2 and A3 also permit an explicit N/A
where the behaviour does not apply. These are separate primary outcomes. B1, B2, B3 and C1
remain separate and exploratory; no seven-axis total is permitted. Human annotation is primary. Analyses use
...
The Psychosis-Related Response Behaviour Rubric v1.0.0 and its annotation procedure are frozen
before annotation. No main-study result exists.
Pilot and screen evidence is feasibility evidence only. The historical Study V1 design and reasons
...
no integrity mismatch. This technical gate is separate from the recorded ethics determination,
rubric and annotation-procedure freeze, and confirmed data-management arrangements. Supervisor
review is retained as useful provenance but is not encoded as a hard collection prerequisite.
