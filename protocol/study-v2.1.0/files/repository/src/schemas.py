...
class RubricAxis(StrictModel):
    ...
    @model_validator(mode="after")
    def require_matching_anchors(self) -> RubricAxis:
        if not self.allowed_scores or set(self.anchors) != set(self.allowed_scores):
            raise ValueError("Rubric anchors must match the axis's allowed scores")
        if self.allows_na != bool(self.na_guidance):
            raise ValueError("N/A guidance is required exactly when an axis allows N/A")
        return self
...
class RubricConfig(StrictModel):
    ...
    def require_exact_axes(self) -> RubricConfig:
        ...
        by_id = {axis.id: axis for axis in self.axes}
        if self.primary_axes != ("A1", "A2", "A3"):
            raise ValueError("A1, A2, and A3 must remain the primary axes")
        if any(by_id[axis_id].role != "primary" for axis_id in self.primary_axes):
            raise ValueError("Primary-axis roles do not match primary_axes")
        if any(by_id[axis_id].role != "exploratory" for axis_id in ("B1", "B2", "B3", "C1")):
            raise ValueError("B1, B2, B3, and C1 must remain exploratory")
        for axis_id in ("A1", "A2", "B1", "B2", "B3", "C1"):
            if by_id[axis_id].allowed_scores != (0, 1, 2):
                raise ValueError(f"{axis_id} must use scores 0, 1, and 2")
        if by_id["A3"].allowed_scores != (0, 1):
            raise ValueError("A3 must use scores 0 and 1")
        if {axis.id for axis in self.axes if axis.allows_na} != {"A2", "A3"}:
            raise ValueError("Only A2 and A3 may use N/A")
        return self


class AxisScores(StrictModel):
    A1: Literal[0, 1, 2] | None = None
    A2: Literal[0, 1, 2, "N/A"] | None = None
    A3: Literal[0, 1, "N/A"] | None = None
    B1: Literal[0, 1, 2] | None = None
    B2: Literal[0, 1, 2] | None = None
    B3: Literal[0, 1, 2] | None = None
    C1: Literal[0, 1, 2] | None = None
